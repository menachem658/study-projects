...
1. loading the complete covid19 dataset in csv format from the world-of-data and changing the class to date
```{r, cache=TRUE}
...
2. Create a function that will use me on finding a maximum for three different columns.
...
3a. Writing a function that recieves as input the data-frame, and a column name as string. The function plots the value of the input column as a function of the date for each of the six continents.
...
3b.Using the function written in a. and plot of the number of `new_cases` for the continents. making a similar plot for the *log* of the *smoothed* number of new cases. 
...
4b.Applying the function from a. to fill the `new_tests_smoothed` column for the continents, and plotting the *log* of the *smoothed* number per continent vs. date using the function from qu. 3. 
```{r, cache=TRUE}
...
5a.  Creating a new data-frame with one row per country, that for each countrie will store as columns the current `total_cases_per_million` and `total_deaths_per_million`, in addition to the country name (`location`), and creating a scatter plot on this data.
...
5b. Finding for each country the date at which the number of new `cases` was maximal, and the date at which the number of new `deaths` was maximal, and creating a scatter plot.
```{r, cache=TRUE}
...
its really make sense that after a few weeks from the date with maximum new cases, it will be the date with the maximum dates (As we will see in question 9).
...
6. computing the world-wide number of `new_cases`, `new_deaths` and `new_vaccinations` by month. 
...
8.investigating whether different countries employ different vaccination strategies
...
9.studying the time delay between the diagnosis of Covid-19 and the death from Covid-19 for cases not surviving the disease.
...
10.examining if the data shows evidence for the effectiveness of the vaccines in reducing the number of Covid-19 cases. 
```{r, cache=TRUE}
